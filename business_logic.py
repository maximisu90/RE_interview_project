...
class business_logic():
    ...
    def apply_logic_bl_energy(self):
        if (float(self.parameters['Beamline energy'].get_cur_val()) > 1200) &\
                (self.parameters['Harmonic'].curr_val.text() == '1st'):
            self.parameters['Harmonic'].set_val.setCurrentIndex(1)
            self.parameters['Harmonic'].curr_val.setText('3rd')
        elif (float(self.parameters['Beamline energy'].get_cur_val()) <= 1200) &\
                (self.parameters['Harmonic'].curr_val.text() == '3rd'):
    ...
    def apply_logic_analyzer_res(self):        
        ...
        d = float(self.parameters["Analyzer slit"].curr_val.text())
        PE = dE*1e-3*300/d
        PE_list = [float(self.parameters["Pass energy"].set_val.itemText(i)) 
                   for i in range(self.parameters["Pass energy"].set_val.count())]
